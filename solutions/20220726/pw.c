   ...
   while ( 1 ){
      ...
      for (i=0,j=0; read(s2,hbuffer+i,1); i++) {
         ...
         if (hbuffer[i]==':' && !h[j].v && j>0){
            hbuffer[i]=0;
            h[j].v = hbuffer + i + 1;
            if(strstr(h[j].n, "Accept-Language")!=NULL){//strstr look for the second param in the first and returns a pointer to the first occurence in the first:)
               languageIndex=j;
            }
         }
      }
      ...
      //select language
      char selectedLanguage[3]="--\0";
      for(int l=0;languageIndex!=-1&&l<strlen(h[languageIndex].v)-1;l++){
         char charToCheck[3];
         charToCheck[0]= h[languageIndex].v[l];
         charToCheck[1]= h[languageIndex].v[l+1];
         charToCheck[2]=0;
         if(strstr(charToCheck, "it")!=NULL) strcpy(selectedLanguage, "it\0");
         if(strstr(charToCheck, "en")!=NULL) strcpy(selectedLanguage, "en\0");         
         if(strstr(charToCheck, "de")!=NULL) strcpy(selectedLanguage, "de\0");
         if(strstr(charToCheck, "fr")!=NULL) strcpy(selectedLanguage, "fr\0");
         if(strstr(charToCheck, "sp")!=NULL) strcpy(selectedLanguage, "sp\0");

         if(selectedLanguage[0]!='-')break;
      }
      if(selectedLanguage[0]=='-')strcpy(selectedLanguage, "en\0"); 
      ...
      if ( !strcmp(method,"GET")){
         ...
         if(strstr(hostname,"openoffice.org")!=NULL){
