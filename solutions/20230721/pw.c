      ...
      if ( !strcmp(method,"GET")){
         ...
         while ( t=read(s3,buffer,2000)){
            if(firstRun==0){
               ...
               firstRun++;
               printf("code: %s <-----------------\n", code);
               if(strcmp("302\0",code) == 0||strcmp("301\0",code) == 0){
                  for(int l=0;l<200;l++)printf("l");
                  for(;k<2000&& !(buffer[k-7]=='L'&&buffer[k-6]=='o'&&buffer[k-5]=='c'&&buffer[k-4]=='a'&&buffer[k-3]=='t'&&buffer[k-2]=='i'&&buffer[k-1]=='o'&&buffer[k]=='n');k++){}
         ...
         if(strcmp("302\0",code) == 0||strcmp("301\0",code) ==0){
            printf(">>>>>>> %s <---------------", newUrl);
            //return new url after retrieving it
            //scheme=newUrl;
            url=newUrl;
            strcpy(scheme,newUrl);
            // GET http://www.aaa.com/file/file
            printf("url=%s\n",url);
            for(i=0;url[i]!=':' && url[i] ;i++);
            if(url[i]==':') url[i++]=0;
